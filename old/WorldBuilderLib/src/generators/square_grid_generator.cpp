...
SquareGridGenerator::SquareGridGenerator(WorldConfig *cfg) { world_cfg_ptr = cfg; }
...
std::vector<TectonicPlate *> SquareGridGenerator::GetTectonicPlates() { return created_plates; }
	...
	for (int i = 0; i < world_cfg_ptr->grid_x_size; i++) {
		for (int j = 0; j < world_cfg_ptr->grid_y_size; j++) {
			...
			if (j < top_equator_id) {
				...
				tileset[i][j]->x_bottom_border_size =
					j + 1 / top_equator_id * universal_border_size;
			} else {
				int dividend = world_cfg_ptr->grid_y_size - j - 1;
				tileset[i][j]->x_bottom_border_size =
					dividend / top_equator_id * universal_border_size;
				tileset[i][j]->x_top_border_size =
					dividend + 1 / top_equator_id * universal_border_size;
			}
		}
	}
...
void SquareGridGenerator::GenerateAndSaveNewTectonicPlates(std::vector<Point> saved_points,
														   bool is_major) {
	for (int i = 0; i < saved_points.size(); i++) {
		TectonicPlate *new_plate = new TectonicPlate();
		new_plate->is_major = is_major;
		/* In the event of plate already being asigned to this tile (only when minor overwrites
		 * major) we take tile to the right if possible if not we assign plate to left one
		 */
		if (generator_tileset[saved_points[i].pos_x][saved_points[i].pos_y]->plate)
			if (world_cfg_ptr->grid_x_size = saved_points[i].pos_x)
				generator_tileset[saved_points[i].pos_x - 1][saved_points[i].pos_y]->plate =
					new_plate;
			else
				generator_tileset[saved_points[i].pos_x + 1][saved_points[i].pos_y]->plate =
					new_plate;
...
void SquareGridGenerator::PopulateGridWithTectonicStartPoints() {
	...
	int max_radius_major =
		(world_cfg_ptr->grid_x_size + world_cfg_ptr->grid_y_size) / plate_counts.major_plates_count;
	int max_radius_minor =
		(world_cfg_ptr->grid_x_size + world_cfg_ptr->grid_y_size) / plate_counts.minor_plates_count;
	int min_radius_major = (world_cfg_ptr->grid_x_size + world_cfg_ptr->grid_y_size) /
						   plate_counts.major_plates_count / RADIUS_DIVIDER;
	int min_radius_minor = (world_cfg_ptr->grid_x_size + world_cfg_ptr->grid_y_size) /
						   plate_counts.minor_plates_count / RADIUS_DIVIDER;

	/* Get major plates starting points */
	std::vector<Point> saved_points =
		GetPlateCoordinates(plate_counts.major_plates_count, max_radius_major, min_radius_major);
	...
	saved_points =
		GetPlateCoordinates(plate_counts.minor_plates_count, max_radius_minor, min_radius_minor);
...
std::vector<Point> SquareGridGenerator::GetPlateCoordinates(int plate_count, int max_radius,
															int min_radius) {
	std::vector<Point> saved_points;
	for (int i = 0; i < plate_count; i++) {
		for (int i = 0; i < world_cfg_ptr->generator_config.poisson_retry_count; i++) {
			...
			if (saved_points.empty()) {
				...
				Point temp_point =
					GetPoissonPointPositiveOnly(saved_points.back(), max_radius, min_radius);
			}
